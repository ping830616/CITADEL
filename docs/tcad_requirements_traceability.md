...
| Stable-graph and ranking robustness | Vary `tau_c` or `pi_min` one at a time and remove each structural ranking term at the four frozen Table VI configurations | `scripts/run_graph_sensitivity.py`, notebook graph-sensitivity cells, `configs/graph_sensitivity.json`, `docs/graph_ranking_sensitivity.md`, `results/notebook_run/graph_sensitivity/` |
...
6. **Graph and ranking robustness:** derive every reported MCC, benign-FPR, and selected-feature-overlap endpoint from `graph_sensitivity_summary.csv`; trace the endpoint to fold rows, selected features, the exact protocol, and hashed inputs in the sensitivity manifest.

Every claim should have a notebook cell, a manifest, a table or figure, and a paragraph-ready interpretation before it enters the paper. A sensitivity range is not traceable unless its minimum and maximum identify the exact setup/event and variant rows that produce them.
