...
| Intel workload-order stress test | Archived-data reanalysis | `uv run --frozen python scripts/reproduce.py intel-orders --verify --repeat`; direct analyzer/notebook use is development-only | Generated: `results/reproduced/<run-id>/notebook_run/intel_workload_orders/`; paper reference: `reproducibility/paper_results/intel/` | `ARCHIVED_CLEAN_VERIFIED` | Two clean complete runs from source commit `c0a31de...` agreed on all 166 scientific CSVs. The compact archive retains the Figure 7 source/summary tables, portrait three-panel figure, both run receipts, both analyzer manifests, 26 input hashes, exact source/environment/runtime evidence, and a passing `0.58% +/- 0.82` / `0.83% +/- 1.00` claim audit. Constructed boundaries test order/distribution sensitivity, not a continuously measured physical switch transient. |
...
reproducibility/paper_results/intel/
  ...
  provenance/
    primary_run_receipt.json
    repeat_run_receipt.json
    primary_analyzer_manifest.json
    repeat_analyzer_manifest.json
