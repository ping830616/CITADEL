...
if [[ -z "${DROOP_ARGS_DDR5}" ]]; then
    echo "Set DROOP_ARGS_DDR5 to the verified arguments from the original DDR5 experiment." >&2
    exit 1
fi
if [[ ! -x "${DROOP_BIN}" ]]; then
...
read -r -a droop_arguments <<< "${DROOP_ARGS_DDR5}"
