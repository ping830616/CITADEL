...
: "${DROOP_DDR4_START_MV:?Set DROOP_DDR4_START_MV in collection_config.sh.}"
: "${DROOP_DDR4_END_MV:?Set DROOP_DDR4_END_MV in collection_config.sh.}"
: "${DROOP_DDR4_STEP_MV:?Set DROOP_DDR4_STEP_MV in collection_config.sh.}"
if (( DROOP_DDR4_START_MV >= 0 || DROOP_DDR4_END_MV >= DROOP_DDR4_START_MV || DROOP_DDR4_STEP_MV <= 0 )); then
    collection_error "DDR4 offsets require a negative start, a more negative end, and a positive step magnitude."
fi
...
printf 'WARNING: writing DDR4 voltage offsets through MSR 0x150.\n' >&2
run_privileged "${DROOP_OPERATION_BIN}" \
    -s "${DROOP_DDR4_START_MV}" \
    -e "${DROOP_DDR4_END_MV}" \
    -v "${DROOP_DDR4_STEP_MV}"
