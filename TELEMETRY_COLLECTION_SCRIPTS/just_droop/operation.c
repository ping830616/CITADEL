...
void reset_voltage_for_signal(int signal_number)
{
	/* pwrite and _exit are safe to use from this signal handler. */
	voltage_change(fd,plane0_zero);
	voltage_change(fd,plane2_zero);
...
int main (int argc, char* argv[])
{
    ...
    */
	...
	signal(SIGINT,reset_voltage_for_signal);
	signal(SIGTERM,reset_voltage_for_signal);
    /*
